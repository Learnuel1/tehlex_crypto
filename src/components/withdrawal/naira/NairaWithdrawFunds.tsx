import { Box, Button,  FormControl, FormLabel, Heading, HStack, Input, Stack, Text, VStack } from '@chakra-ui/react'
import React, { useState } from 'react'
import { IoIosArrowRoundBack } from 'react-icons/io'
import { useNavigate } from 'react-router-dom';

const NairaWithdrawFunds = () => {
    ...
    const handlwWithdraw = (e:React.FormEvent) => {
        e.preventDefault()
        navigate("/dashboard/nairapaymentmethod")
    }
  return (
    <Box maxW={'100vw'} minH='30vw' p='2rem'>
        ...
        <IoIosArrowRoundBack size={'2.5rem'} color='blue' 
            onClick={() =>navigate(-1) }
        />            
        <Heading size={'md'} textAlign={'center'}>Withdraw Funds In Naira</Heading>
        </Stack>

       <VStack gap={'2rem'}>
        <Heading>N {withdraw}</Heading>
...
export default NairaWithdrawFunds
